 ...
 * Fixed: Realtime Database support for Products, Variations, and Deletion.
 */
...
if ($path === 'products' && $method === 'POST') {
    ...
    $productData = [
        ...
        "price" => "0", // ভ্যারিয়েশন না আসা পর্যন্ত ডিফল্ট
        "images" => $images,
        "tags" => $inputData['tags'] ?? [],
        "created_at" => $now,
        "updated_at" => $now
...
if (preg_match('/products\/(.+)\/variations/', $path, $matches) && $method === 'POST') {
    ...
    if (!empty($vars)) {
        // প্রথম ভ্যারিয়েশনের দামকে মেইন দাম হিসেবে আপডেট করা
        $mainPrice = $vars[0]['regular_price'] ?? "0";
        firebaseRequest("PATCH", "/products/$parentPid.json", ["price" => (string)$mainPrice]);

        foreach ($vars as $index => $v) {
            $varId = $parentPid . "00" . $index;
            $varData = [
                "sku" => $v['sku'],
                "price" => (string)$v['regular_price'],
                "image" => $v['image'],
                "name" => $v['properties'][0]['value'] ?? 'Option'
            ];
